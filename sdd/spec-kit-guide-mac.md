# GitHub Spec-Kit macOS + Claude Code CLI 전용 SDD 개발 가이드

> **Spec-Driven Development (SDD)**: 명세(Specification)가 실행 가능해지고, 코드 구현을 직접 생성하는 개발 방식
>
> **플랫폼**: macOS + Claude Code CLI 전용 가이드
>
> **작성일**: 2026-04-20
...
3. [macOS에서 Spec-Kit 설치하기](#3-macos에서-spec-kit-설치하기)
4. [Claude Code CLI와 프로젝트 초기화](#4-claude-code-cli와-프로젝트-초기화)
5. [6단계 워크플로우 완전 정복](#5-6단계-워크플로우-완전-정복)
6. [macOS 폴더 구조 이해하기](#6-macos-폴더-구조-이해하기)
7. [실전 예제: macOS + Claude Code로 할 일 앱 만들기](#7-실전-예제-macos--claude-code로-할-일-앱-만들기)
8. [Extensions & Presets](#8-extensions--presets)
9. [macOS 초보자를 위한 핵심 팁](#9-macos-초보자를-위한-핵심-팁)
10. [macOS 유용한 명령어 모음](#10-macos-유용한-명령어-모음)
...
## 3. macOS에서 Spec-Kit 설치하기

### 3.1 macOS 터미널 열기

- **Spotlight**: `Cmd + Space` → "Terminal" 검색 → Enter
- **iTerm2**를 사용하면 더 나은 경험을 할 수 있습니다: `brew install --cask iterm2`

### 3.2 Homebrew 설치 (패키지 관리자)
...
> **Apple Silicon (M1/M2/M3) Mac**의 경우 Homebrew가 `/opt/homebrew`에 설치됩니다.
> **Intel Mac**의 경우 `/usr/local`에 설치됩니다.

### 3.3 Git 설치 확인

macOS에는 기본적으로 Git이 포함되어 있지만, Homebrew로 최신 버전을 설치할 수 있습니다:
...
macOS에는 Python이 기본 설치되어 있지 않을 수 있습니다:
...
### 3.6 Claude Code CLI 설치

```bash
# npm이 설치되어 있어야 합니다 (Homebrew로 설치 가능)
brew install node
...
브라우저가 열리면서 Anthropic 계정 로그인이 필요합니다.
...
```bash
# bash 스크립트(.sh)를 명시적으로 생성하도록 지정
specify init my-app --ai claude --script sh
```

> macOS는 기본적으로 zsh를 사용하지만, Spec-Kit 스크립트는 bash 호환으로 생성됩니다.
...
│   ├── scripts/
│   │   ├── create-new-feature.sh          ← 새 기능 생성 bash 스크립트
│   │   └── setup-template.sh              ← 템플릿 설정 bash 스크립트
│   └── templates/
...
> **macOS 참고**: bash 스크립트(`.sh`)가 생성되며, zsh에서도 호환됩니다.
...
Claude Code가 `[NEEDS CLARIFICATION]` 마커로 표시된 부분을 질문으로 바꿔줍니다.
...
tasks.md의 태스크를 **순서대로** 실행합니다. Claude Code가 코드를 직접 작성합니다.
...
# 최근 수정된 파일 확인 (최근 10개)
find . -name "*.md" -mtime -1 | xargs ls -lt | head

# specs 폴더 트리 구조 보기
tree specs  # tree가 없으면: brew install tree
```

---

## 7. 실전 예제: macOS + Claude Code로 할 일 앱 만들기
...
> **Claude Code**: "이 프로젝트의 핵심 원칙을 정립하겠습니다. 다음 질문에 답해주세요:
...
```bash
cat specs/001-todo-app/spec.md
```
...
> **Claude Code**: "spec.md에서 다음 부분이 모호합니다:
> 1. 할 일의 마감일이 필요한가요?
> 2. 사용자 인증이 필요한가요?
> 3. 카테고리 분류 기능이 있나요?"
...
Claude Code가 태스크를 하나씩 실행합니다. 각 태스크 완료 후 터미널에서 Git 커밋:
...
기능이 바뀌면 명세도 바꿉니다. 코드를 먼저 바꾸지 말고, 명세를 먼저 수정한 후 Claude Code가 코드를 재생성하게 하세요.

### Tip 6: 병렬 태스크 활용

`[P]` 마커가 붙은 태스크는 동시에 진행할 수 있습니다. 여러 AI 에이전트를 쓰면 더 빠릅니다.

### Tip 7: Claude Code CLI 단축키 활용

| 단축키 | 기능 |
|--------|------|
| `Ctrl + C` | Claude Code 작업 중단 |
| `↑` / `↓` | 이전/다음 프롬프트 히스토리 |
| `/help` | 도움말 |
| `/clear` | 대화 내용 초기화 |
| `/exit` | Claude Code 종료 |
...
macOS Catalina 이후 기본 셸은 **zsh**입니다. `.zshrc`에 다음을 추가하면 Claude Code 작업이 편해집니다:

```bash
# .zshrc에 추가
alias claude-dev="claude --dangerously-skip-permissions"
alias spec-kit="cd ~/projects/spec-kit-app && claude"
```
...
### Tip 9: VS Code와 함께 사용하기

```bash
# VS Code CLI 설치 (Command Palette에서 설치 가능)
# VS Code에서: Cmd + Shift + P → "Shell Command: Install 'code' command"
...
# Homebrew 필수 패키지
brew install git node python@3.12 uv tree

# Claude Code 최신 버전 유지
npm update -g @anthropic-ai/claude-code
```

### Tip 11: Claude Code 메모리 활용

Claude Code는 `.claude/` 폴더에 프로젝트 컨텍스트를 저장합니다:
...
프로젝트별 지침을 `.claude/CLAUDE.md`에 작성하면 Claude Code가 매번 참고합니다.

### Tip 12: macOS Spotlight로 빠른 접근

macOS에서 개발 관련 경로를 빠르게 열려면:
...
# 자주 쓰는 디렉토리 심볼릭 링크
ln -s ~/Projects/todo-app/specs ~/Desktop/specs-link
```
...
# 가장 최근에 수정된 명세 파일
find specs -name "*.md" -exec stat -f "%m %N" {} \; | sort -rn | head -5 | cut -d' ' -f2- | xargs ls -la
```
...
```bash
# 새 기능 생성 스크립트 실행
bash .specify/scripts/create-new-feature.sh --name "user-profile"

# 템플릿 설정 스크립트 실행
bash .specify/scripts/setup-template.sh
```

### Claude Code 유용한 슬래시 명령어
...
## 부록: macOS별 자주 묻는 질문 (FAQ)

### Q: Spec-Kit은 무료인가요?
A: 네. MIT 라이선스의 오픈소스입니다.

### Q: macOS에서 Claude Code 외에 어떤 AI 코딩 도구와 호환되나요?
A: GitHub Copilot CLI, Gemini CLI, Codex CLI, Cursor, Windsurf, OpenCode 등을 지원합니다.

### Q: Claude Code CLI가 터미널에서 인식되지 않아요.
A: `npm install -g @anthropic-ai/claude-code`로 재설치하고, `echo $PATH`에서 npm global bin 경로가 포함되어 있는지 확인하세요. `npm bin -g`로 경로를 확인할 수 있습니다.
...
### Q: zsh과 bash 중 무엇을 써야 하나요?
A: macOS 기본 셸인 **zsh**를 사용하세요. `.sh` 스크립트는 zsh에서도 잘 동작합니다. 호환성 문제가 있으면 `bash`로 명시 실행하세요.

### Q: Apple Silicon (M1/M2/M3)에서 잘 동작하나요?
A: 네. uv, Node.js, Python 모두 Apple Silicon을 네이티브로 지원합니다. Rosetta 2가 필요하지 않습니다.

### Q: Claude Code의 토큰 사용량이 걱정됩니다.
A: `/usage` 명령어로 현재 세션의 사용량을 확인하세요. 컨텍스트가 길어지면 `/compact`로 압축하거나, 큰 명세 파일을 먼저 완성하고 새 세션에서 `/speckit.implement`를 실행하는 것을 권장합니다.
...
- macOS + Claude Code 조합은 터미널 기반 개발에 최적화되어 있습니다. VS Code 터미널에서 Claude Code를 실행하면 에디터와 터미널을 오가지 않고 효율적으로 작업할 수 있습니다.
`─────────────────────────────────────────────────`
